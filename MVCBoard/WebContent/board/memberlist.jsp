...
<c:choose>
	...
	<a href='<c:url value="/Board.do?action=memberList&page=${startPage-1}"/>'>◀</a>
	</c:if>
	<c:forEach begin="${startPage}" end="${endPage}" var="i">
	[<a href='<c:url value="/Board.do?action=meberList&page=${i}"/>'>${i}</a>]
	</c:forEach>
	<c:if test="${nowPageBlock lt totalPageBlock}">
	<a href='<c:url value="/Board.do?action=meberList&page=${endPage+1}"/>'>▶</a>
	</c:if>
</h6>
<!-- 본문 표시 코드 -->
<tr height="50"><td>
